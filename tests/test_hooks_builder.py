...
from hooks_builder import validate_inline_hooks_config  # noqa: E402
...
def _hooks_payload() -> dict:
    apps_payload = parse_toml_file(APPS_TOML_PATH)
    hooks = apps_payload.get("hooks")
...
class HookBuilderTests(unittest.TestCase):
    def test_validate_inline_hooks_config_accepts_repo_layout(self) -> None:
        hooks = validate_inline_hooks_config(
            APPS_TOML_PATH,
            HOOK_SCRIPTS_DIR,
    ...
    def test_validate_inline_hooks_config_rejects_tool_matcher_drift(self) -> None:
    ...
    def test_validate_inline_hooks_config_rejects_subagent_command_drift(self) -> None:
        hooks = copy.deepcopy(_hooks_payload())
