#!/usr/bin/env python3
"""Utility helper for the health-adhd-cbt skill.
...
SKILL_NAME = 'health-adhd-cbt'
SKILL_CATEGORY = 'HEALTH'
SKILL_DESCRIPTION = 'Comprehensive ADHD planning and CBT-based (sometimes requested as "CBD") daily templates with PDF generation workflows. Use when generating printable ADHD/CBT worksheets, daily planners, impulse-control decision filters, emotion regulation check-ins, or when converting template content to PDF/HTML using the bundled scripts and assets.'
...
REFERENCE_LINKS = [
    {'title': 'NIMH ADHD overview', 'url': 'https://www.nimh.nih.gov/health/topics/attention-deficit-hyperactivity-disorder-adhd', 'note': 'High-level ADHD guidance and references.'},
    {'title': 'CDC ADHD resources', 'url': 'https://www.cdc.gov/adhd/index.html', 'note': 'Public-health information for ADHD planning context.'},
    {'title': 'NIMH Psychotherapies', 'url': 'https://www.nimh.nih.gov/health/topics/psychotherapies', 'note': 'Psychotherapy context and evidence framing.'},
    {'title': 'NICE ADHD guidance', 'url': 'https://www.nice.org.uk/guidance/ng87', 'note': 'Clinical guideline reference for checklist framing.'},
    {'title': 'WHO mental health resources', 'url': 'https://www.who.int/health-topics/mental-health', 'note': 'Global mental health context.'},
    {'title': 'OWASP ASVS V5', 'url': 'https://owasp.org/www-project-application-security-verification-standard/', 'note': 'Input validation and safe file processing controls for renderer scripts.'}
]
...
def cmd_validate() -> int:
    ...
    required_scripts = [
        skill_root() / 'scripts' / 'render_template.py',
        skill_root() / 'scripts' / 'build_pack.py',
        skill_root() / 'scripts' / 'build_daily_pack.py',
    ]
    for script_path in required_scripts:
        if not script_path.is_file():
            errors.append(f"missing required script: {script_path}")
