...
Use this file format with `../scripts/build_pack.py` from the skill root.
...
- `css` (optional): CSS path relative to the skill root `../assets/` directory (must not escape root) or an absolute path.
- `pages` (required): array of page entries (see below).

## Page entry fields
- `template` (required): template filename or path. If no extension, `.html` is added.
  Relative paths are resolved under `../assets/templates/` and may not escape that directory.
