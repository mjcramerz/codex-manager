---
name: desktop-librewolf
description: Configure LibreWolf with hardened privacy and security defaults for day-to-day
  desktop usage. Use when the user explicitly asks for LibreWolf setup or policy tuning.
metadata:
  version: '1.0'
  short-description: Configure LibreWolf with hardened defaults
  tags:
  - librewolf
  - browser
  - privacy
interface:
  display-name: DESKTOP-LibreWolf
  short-description: Configure LibreWolf with hardened defaults
  icon-small: assets/icon-32.png
  icon-large: assets/icon-128.png
  brand-color: '#6BCC32'
  default-prompt: Act as the "DESKTOP-LibreWolf" specialist for "Configure LibreWolf with
    hardened defaults". Deliver focused, deterministic results with minimal, reviewable changes
    and explicit assumptions. Validate untrusted inputs and bounded I/O, run the narrowest
    relevant checks, and report concrete actions, evidence, and residual risks.
---

## Use this skill when
- setting LibreWolf overrides or policies
- wiring Wayland launch flags

## Workflow
1) Confirm install provenance
2) Apply overrides/policies
3) Validate profile permissions

## Agent orchestration
- Confirm that the request fits this skill and state boundaries if other skills are needed.
- For multi-step work, keep a concise plan, execute in small reversible steps, and surface assumptions early.
- Delegate only independent discovery tasks, then reconcile findings before making edits.

## Validation and testing
- Validate critical inputs and bound external I/O (size, retries, and timeouts) before applying changes.
- Run the narrowest relevant checks that prove behavior (tests, lint, or build as applicable).
- Include risk-based negative or edge-case coverage for security-sensitive, parsing, or automation changes.
- Report verification commands, outcomes, and any follow-up checks that remain.

## Outputs

- Actionable steps or artifacts aligned to the skill.
- References to relevant files, commands, or templates.

## References
- `$CODEX_HOME/index/domains/desktop/librewolf.md`
- `$CODEX_HOME/docs/desktop/librewolf.md`
- `$CODEX_HOME/snippets/desktop/librewolf.overrides.cfg`
