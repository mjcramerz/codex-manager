# Vscode (domain router, overview)
Purpose: quick navigation for this domain’s entrypoints.


## Contents
<!-- BEGIN:contents -->
- `$CODEX_HOME/index/domains/vscode/guidance.md` — VS Code guidance (entrypoint)
- `$CODEX_HOME/index/domains/vscode/vscode-extension.md` — VS Code extension workflow (entrypoint)
<!-- END:contents -->


## Navigation
<!-- BEGIN:nav -->
- Parent: `$CODEX_HOME/index/domains/overview.md`
- Pack index: `$CODEX_HOME/INDEX.md`
- Routing guide: `$CODEX_HOME/index/OVERVIEW.md`
<!-- END:nav -->

Entry points:
- `$CODEX_HOME/index/domains/vscode/guidance.md` — stable link to VS Code guidance.
- `$CODEX_HOME/index/domains/vscode/vscode-extension.md` — stable link to VS Code extension workflow.
