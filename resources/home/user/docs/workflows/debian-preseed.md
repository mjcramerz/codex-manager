...
Purpose: guide unattended Debian installation work, with concrete alignment to the `debian-preseed-di` repository for the Codex coding agent.
...
- You must keep preseed files under version control but never store plaintext passwords.
- You must use HTTPS for hosted preseeds and restrict access where possible.
...
## Passwords
- You must use `passwd/user-password-crypted` with a strong hash.
...
- You must keep storage, class-selection, and late-command behavior grounded in the tracked repo contracts.
- You must treat kernel command-line inputs as untrusted and validate shape early.
- You must keep secrets out of tracked seeds; prefer deployment-time injection for sensitive values.
...
- You must keep `preseed.cfg` minimal and include split seeds.
...
- You must use hashed credentials only and protect hosted preseed delivery with access controls.
- Pin destructive disk targets explicitly (`partman-auto/disk`) for each hardware class.
- You must treat late-command assets as sensitive and rotate bootstrap secrets after install.

## Testing checkpoints
- You must run unattended installs in VMs that match BIOS/UEFI modes used in production.
- You must verify partition layout, account policy, package set, and bootloader state post-install.
...
- You must keep prior preseed revision and boot params for rollback in provisioning systems.
