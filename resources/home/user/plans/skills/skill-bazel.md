# Plan
Purpose: tell the Codex coding agent how to use `plans/skills/skill-bazel.md` as a runtime-pack surface and when to stop browsing.

You must use this plan when applying or updating the `bazel` skill.
...
## Scope
- In: tasks covered by the `bazel` skill and its resources.
- Out: tasks outside the skill’s domain.

## Action items
[ ] Use skill `bazel` and linked resources.
